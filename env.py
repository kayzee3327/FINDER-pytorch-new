...
class FinderEnvironment:
    def __init__(self, n_step, tensor_type=torch.float32, measurement="pairwise connectivity"):
        self.G = None
        self.state_sequence = []
    ...
    def load_graph(self, graph: nx.Graph()):
        self.G = graph.copy()
        self.state_sequence = [torch.tensor(nx.adjacency_matrix(graph).todense(), dtype=self.tensor_type)]  # s_0
        self.action_sequence = []
        self.reward_sequence = []
    ...
    def step(self, action):
        ...
        self.state_sequence.append(torch.tensor(nx.adjacency_matrix(self.G).todense(), dtype=self.tensor_type))  # s_t+1
